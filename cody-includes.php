...
function schema_tax(){
    register_taxonomy('schema', ['schema_pgc', 'schema_structure', 'schema_hfw'], [
    ...
    register_taxonomy_for_object_type( 'schema', ['schema_pgc', 'schema_structure']);
...
// $which (the position of the filters form) is either 'top' or 'bottom'
    add_action('restrict_manage_posts', function ($post_type, $which) {
        if ('top' === $which && in_array($post_type, ['schema_pgc', 'schema_structure', 'schema_hfw'])) {
            $taxonomy = 'schema';
